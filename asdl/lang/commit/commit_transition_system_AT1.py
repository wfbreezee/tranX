...
def commit_node_to_ast(grammar, commit_tokens, start_idx):
    ...
    elif node_name[0].isdigit():
        # it's a time
        prod = grammar.get_prod_by_ctr_name('Time')
        ast_node = AbstractSyntaxTree(prod,
                                      [RealizedField(prod['time'], value=node_name)])
        i += 1
    elif node_name.startswith('/'):
        # it's a path
        prod = grammar.get_prod_by_ctr_name('Path')
        ast_node = AbstractSyntaxTree(prod,
                                      [RealizedField(prod['path'], value=node_name)])

        i += 1
    elif node_name is not None:
        # it's a name
        prod = grammar.get_prod_by_ctr_name('Name')
        ast_node = AbstractSyntaxTree(prod,
                                      [RealizedField(prod['name'], value=node_name)])
...
def commit_expr_to_ast_helper(grammar, commit_tokens, start_idx=0):
    ...
    while True:
        if commit_tokens[i] == '\\+':
            ...
            i += 1
            if commit_tokens[i] == '(':
                arg_ast_node, end_idx = commit_expr_to_ast_helper(grammar, commit_tokens, i)
            else:
                arg_ast_node, end_idx = commit_node_to_ast(grammar, commit_tokens, i)
            i = end_idx
        ...
        if commit_tokens[i] == ',':
            ...
            i += 1
            right_ast_node, end_idx = commit_expr_to_ast_helper(grammar, commit_tokens, i)
...
def is_equal_ast(this_ast, other_ast):
    ...
    if isinstance(this_ast, AbstractSyntaxTree):
        ...
        for i in range(len(this_ast.fields)):
            if this_ast.production.constructor.name == 'And' and this_ast.fields[i].name == 'arguments':
...
class CommitTransitionSystemtest(TransitionSystem):
...
if __name__ == '__main__':
    commit_expr = 'before ( TIME ) , author ( NAME )'
    grammar = ASDLGrammar.from_text(open('/asdl/lang/commit/commit_asdl_AT1.txt').read())
    ast = commit_expr_to_ast(grammar, commit_expr)
    print(ast_to_commit_expr(ast))
